...
const useStyles = makeStyles((theme) => ({
  ...
  const validate = values => {
    ...
    if (!values.directiondesc) {
      errors.directiondesc = 'Required';
    }
    return errors;
  };
  ...
  const App = view(() => {
    ...
    const onSubmit =  async values   => {
      const sleep = ms => new Promise(resolve => setTimeout(resolve, ms));
        ...
        try {
          var x =  (await axios.post(AppConfig.API+`Direction/Add`, values ,{ headers: JSON.parse( window.localStorage.getItem("ldat"))})).status;
        ...
        if(appStore.edit){
         ...
         try {
            var y =  (await axios.put(AppConfig.API+`Direction/Update/`+appStore.data[0].id, values)).status;
            if(y == 200){
                ...
                <Grid container alignItems="flex-start" spacing={2}>
                  ...
                  <Grid item xs={12}>
                    <Field
                      ...
                      type="text"
                      label="Direction"
                      initialValue={  appStore.data.length!=0 ?  appStore.data[0].directiondesc : ""}
                    />
                  </Grid>
