...
const columns = [
  ...
  { field: "comment", headerName: "Comment", flex: 0.2 },
  { field: "dateComment", headerName: "Date" , flex :0.1 },
  { field: "userComment", headerName: "User", flex: 0.2 ,valueGetter : ({ value }) => value? value.nom + " " + value.prenom  : ""},
  { field: "documentComment", headerName: "Ref", flex: 0.2 , valueGetter : ({ value }) => value? value.ref : ""},
  { field: "documentComment", headerName: "Nom", flex: 0.2 , valueGetter : ({ value }) => value? value.nom : ""},
  {
    field: "color",
    headerName: "Action",
    flex: 0.1,
...
async  function DeleteRequest(){
 ...
 try {
    var status =  ( await axios.delete(AppConfig.API+`Comment/Delete/`+appStore.data[0].id ,{ headers: JSON.parse( window.localStorage.getItem("ldat"))}) ).status;
