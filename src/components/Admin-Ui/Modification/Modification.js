...
const columns = [
  ...
  { field: "documentMod", headerName: "Document", flex: 0.4  , valueGetter : ({ value }) => value? value.ref + " "+ value.nom : ""},
  { field: "dateModification", headerName: "Date ", flex: 0.2  },

  { field: "userMod", headerName: "User", flex: 0.2 ,valueGetter : ({ value }) =>value? value.nom+" "+ value.prenom : ""},
  {
    field: "color",
    headerName: "Action",
    flex: 0.1,
...
async  function DeleteRequest(){
 ...
 try {
    var status =  ( await axios.delete(AppConfig.API+`Modif/Delete/`+appStore.data[0].id ,{ headers: JSON.parse( window.localStorage.getItem("ldat"))}) ).status;
