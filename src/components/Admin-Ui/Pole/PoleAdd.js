  ...
  const App = view(() => {
    ...
    const onSubmit =  async values   => {
      const sleep = ms => new Promise(resolve => setTimeout(resolve, ms));
       ...
       try {
          var x =  (await axios.post(AppConfig.API+`Pole/Add`, values ,{ headers: JSON.parse( window.localStorage.getItem("ldat"))})).status;
        ...
        try {
            ...
            var y =  (await axios.put(AppConfig.API+`Pole/Update/`+appStore.data[0].id, values,{ headers: JSON.parse( window.localStorage.getItem("ldat"))})).status;
            if(y == 200){
                ...
                <Grid container alignItems="flex-start" spacing={2}>
                  ...
                  <Grid item xs={12}>
                    <Field
                      ...
                      type="text"
                      label="Pole"
                      initialValue={  appStore.data.length!=0 ?  appStore.data[0].pole : ""}
                    />
                  </Grid>
