  ...
  const App = view(() => {
    ...
    const onSubmit =  async values   => {
      const sleep = ms => new Promise(resolve => setTimeout(resolve, ms));
        ...
        var x =  (await axios.post(AppConfig.API+`Delivrable/Add`, values)).status;
        ...
        if(appStore.edit){
          ...
          var y =  (await axios.put(AppConfig.API+`Delivrable/Update/`+appStore.data[0].id, values)).status;
          if(y == 200){
