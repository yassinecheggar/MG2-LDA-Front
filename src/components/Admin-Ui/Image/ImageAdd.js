  ...
  const App = view(() => {
    ...
    const onSubmit =  async values   => {
      const sleep = ms => new Promise(resolve => setTimeout(resolve, ms));
        ...
        var x =  (await axios.post(AppConfig.API+`Picture/Add`, values)).status;
        ...
        if(appStore.edit){
          ...
          var y =  (await axios.put(AppConfig.API+`Picture/Update/`+appStore.data[0].id, values)).status;
          if(y == 200){
                ...
                <Grid container alignItems="flex-start" spacing={2}>
                  ...
                  <Grid item xs={12}>
                    <Field
                      ...
                      type="text"
                      label="Lien"
                      initialValue={  appStore.data.length!=0 ?  appStore.data[0].link : ""}
                    />
                  </Grid>
